...
class MediaStateUpdater(private val context: Context) {
    ...
    private fun updatePlaybackState(mediaSession: MediaSessionCompat, info: MediaInfo) {
        ...
        val advertisedActions = if (swapEnabled) {
            SWAPPED_ACTIONS
        } else {
            DEFAULT_ACTIONS
        }

        val actions = info.sourceActions?.let { advertisedActions and it } ?: advertisedActions
        val canSeek = info.sourceActions?.let { it and PlaybackStateCompat.ACTION_SEEK_TO != 0L } ?: true
        val stateBuilder = PlaybackStateCompat.Builder()
            .setActions(actions)
        if (canSeek) { stateBuilder.addCustomAction(createRewindAction()); stateBuilder.addCustomAction(createFastForwardAction()) }
